        ...
        if (isJvmRefer) {
            ...
            if (urls.size() == 1) {
                ...
                for (URL url : urls) {
                    //获得zk上的invoker
                    //TODO 要看
                    //refprotocol.refer(interfaceClass, url)返回的是
                    // MockClusterInvoker(RegistryDirectory,FailoverClusterInvoker)
                    invokers.add(refprotocol.refer(interfaceClass, url));
                    if (Constants.REGISTRY_PROTOCOL.equals(url.getProtocol())) {
                        registryURL = url; // use last registry url
                    }
                }
                if (registryURL != null) { // 对有注册中心的Cluster 只用 AvailableCluster
